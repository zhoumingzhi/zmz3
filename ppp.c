...
#else
#define VAR_POS     SQR( 10.0) /* initial variance of receiver position (m^2) */
#define VAR_CLK     SQR( 10.0) /* initial variance of receiver clock (m^2) */
#define VAR_ZTD     SQR(  0.5) /* initial variance of ztd (m^2) */
#define VAR_GRA     SQR(0.001) /* initial variance of gradient (m^2) */
#define VAR_BIAS    SQR(100.0) /* initial variance of phase-bias (m^2) */
#endif
...
static void testeclipse(const obsd_t *obs, int n, const nav_t *nav, double *rs)//这个函数很不懂啊
{
                  ...
                  double phw, double *meas, double *var)
{
    ...
    if (obs->code[i]==CODE_L1C) P1+=P1_C1; /* C1->P1 */ //这个地方只选取了L1C和L1P两种码观测值，其他的不算
                    ...
                    double *meas, double *var, int *brk)
{
    ...
    /* antenna phase center variation correction *///不需要再组合了
    for (i=0;i<2;i++) {
...
static void udtrop_ppp(rtk_t *rtk)
{
    ...
    if (rtk->x[i]==0.0) {
        ecef2pos(rtk->sol.rr,pos);
        ztd=sbstropcorr(rtk->sol.time,pos,azel,&var);//这个如何定义啊
        initx(rtk,ztd,var,i);
                   ...
                   double *H, double *R, double *azel)
{
    ...
    trace(5,"x=(%3d,%3d)\n",1,nx); tracemat(5,x, 1,nx,8,3);
    trace(5,"v=(%3d,%3d)\n",1,nv); tracemat(5,v, 1,nv,8,3);
    trace(5,"H=(%3d,%3d)\n",nx,nv); tracemat(5,H,nx,nv,8,3);
    trace(5,"R=(%3d,%3d)\n",nv,nv); tracemat(5,R,nv,nv,8,5);
    return nv;
}
...
extern void pppos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
    ...
    for (i=0;i<rtk->opt.niter;i++) {
        ...
        if ((info=filter(xp,Pp,H,v,R,rtk->nx,nv,NULL,NULL))) {
