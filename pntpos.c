                     ...
                     int iter, const prcopt_t *opt, double *var)
{
    ...
    *var=0.0;
    ...
    else { /* single-frequency *///广播星历是计算到对应的钟差        
...
static int rescode(int iter, const obsd_t *obs, int n, const double *rs,
                   ...
                   double *resp ,double *dtrop,double * dion,int *nx)
{
    double r,d_ion,d_trp,vmeas,vion,vtrp,rr[3],pos[3],dtr,e[3],P;
	int i, j, nv = 0, ns[2] = { 0 }, sys;
	double sat_var=0;
    
    trace(3,"resprng : n=%d\n",n);
    //fp_residuals=fopen("residuals.txt","a");
    *nx=5;
    //if(!strcmp(time_str(obs[0].time,3),"2014/04/18 20:44:59.923"))
    //    printf("");
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        vsat[i]=0; azel[i*2]=azel[1+i*2]=resp[i]=dtrop[i]=dion[i]=0.0;
        ...
        /* psudorange with code bias correction *///进行了TGD改正
        ...
        if (!ionocorr(obs[i].time,nav,obs[i].sat,pos,azel+i*2,
                      iter>0?opt->ionoopt:IONOOPT_BRDC,&d_ion,&vion)) continue;
        
        /* tropospheric corrections */
        if (!tropcorr(obs[i].time,nav,pos,azel+i*2,
                      iter>0?opt->tropopt:TROPOPT_SAAS,&d_trp,&vtrp)) {//天顶方向
            continue;
        }
        /* pseudorange residual */
        
        v[nv]=P-(r+dtr-CLIGHT*dts[i*2]+d_ion+d_trp);
        //if(i==2){
            
        //}
		if (!strcmp(time_str(obs[i].time, 3), "2014/06/20 00:00:40.000")&&nv==12)
			printf("");
        ...
        if (sys==SYS_CMP) {v[nv]-=x[4]; H[4+nv*5]=1.0; ns[1]++;}//与GPS时间的差值；
        ...
        dtrop[i]=d_trp;
        /* error variance */
		sat_var = varerr(opt, azel[1 + i * 2], sys);
		//if (33<=obs->sat&&obs->sat<=37) sat_var *= 2;
		var[nv++] = sat_var + vare[i] + vmeas + vion + vtrp;
		
    //	printf("sat=%2d,P-r=%13.4f,v=%6.3f,dion=%6.3f,dtrp=%6.3f,var=%6.3f,dtr=%7.3f\n",obs[i].sat,P-r,v[nv-1],d_ion,d_trp,var[nv-1],dtr);
        trace(4,"sat=%2d azel=%5.1f %4.1f res=%7.3f sig=%5.3f\n",obs[i].sat,
              azel[i*2]*R2D,azel[1+i*2]*R2D,resp[i],sqrt(var[nv-1]));
    }


	if (!strcmp(time_str(obs[i].time, 3), "2014/06/20 00:00:40.000"))
		printf("");
...
static int valsol(const double *azel, const int *vsat, int n,
                  const prcopt_t *opt, const double *v, int nv, int nx,sol_t *sol,
                  char *msg)
{
...
static int estpos(const obsd_t *obs, int n, const double *rs, const double *dts,
                  ...
                  double *resp, double *dtrop,double *dion,char *msg)
{
    ...
    time2str(obs->time,s,timeu);
    //if(!strcmp(s,"2014/04/18 20:44:5"))
    for (i=0;i<3;i++) x[i]=sol->rr[i];
    //printf("%s\n",s);
    for (i=0;i<MAXITR;i++) {
        //var是卫星方差
        /* pseudorange residuals */
        nv=rescode(i,obs,n,rs,dts,vare,svh,nav,x,opt,v,H,var,azel,vsat,resp,dtrop,dion,&nx);
        
        if (nv<nx) {
            sprintf(msg,"lack of valid sats ns=%d",nv);  //将一段话复制到一个字符串中
            sol->SPPlabel|=SPP_Lack;
            break;
        }
        /* weight by variance */
        for (j=0;j<nv;j++) {
        ...
        if (norm(dx,nx)<1E-4) {
        //	printf("sig=%6.3f\n",sqrt(SQR(norm(v,nv)/(nv-4))));
        //for ( i=0;i<4;i++) {
        //for ( j=0;j<4;j++) printf(" %9.3f",sqrt(SQR(norm(v,nv)/(nv-4)))*Q[j+4*i]);
        ////printf("%13.3f\t%13.3f",[i],x[i]);
        //printf("\n");}
   // }
            //for(i=0;i<nv;i++)
            //	printf("v[%2d]=%4.3f\n",i+1,v[i]);
            sol->type=0;
            sol->time=timeadd(obs[0].time,-x[3]/CLIGHT);//这个时间已经做了改正；
            sol->dtr[0]=x[3]/CLIGHT; /* receiver clock bias (s) */
            sol->dtr[1]=x[4]/CLIGHT; /* glonass-gps time offset (s) *///时间偏差
            ...
            //sol->rms=sqrt(dot(resp,resp,n)/nv);
            /* validate solution */
            if ((stat=valsol(azel,vsat,n,opt,v,nv,nx,sol,msg))) {
                    ...
                    double *azel, int *vsat, double *resp, char *msg)
{
    ...
    for (i=0;i<n;i++) {
        ...
        if (!estpos(obs_e,n-1,rs_e,dts_e,vare_e,svh_e,nav,opt,&sol_e,azel_e,
                    vsat_e,resp_e,dtrop_e,dion_e,msg_e)) {
...
*          and receiver bias)
*-----------------------------------------------------------------------------*/
//自己将const obsd_t *obs改成obsd_t *obs,
extern int pntpos(const  obsd_t *obs, int n, const nav_t *nav,
                  const prcopt_t *opt, sol_t *sol, double *azel, ssat_t *ssat,
                  char *msg)
{
    prcopt_t opt_=*opt;//没有改变它的值
    double *rs,*dts,*var,*azel_,*resp,*dtrop,*dion;
    ...
    satposs(sol->time,obs,n,nav,opt_.sateph,rs,dts,var,svh);//var端的方差
    
    /* estimate receiver position with pseudorange */
    stat=estpos(obs,n,rs,dts,var,svh,nav,&opt_,sol,azel_,vsat,resp,dtrop,dion,msg);
    ...
    if (ssat) {
        ...
        for (i=0;i<n;i++) {
            //obs[i]
            //if(sol->SPPlabel&SPP_Lack){
            //    ssat[obs[i].sat-1].azel[0]=0;
            //    ssat[obs[i].sat-1].azel[1]=0;

            //}
            //else
                ssat[obs[i].sat-1].azel[0]=azel_[  i*2];
                ssat[obs[i].sat-1].azel[1]=azel_[1+i*2];
                ssat[obs[i].sat-1].mp[0]=obs[i].Mp[0];
                ssat[obs[i].sat-1].mp[1]=obs[i].Mp[1];
                //ssat[obs[i].sat-1].mp[1]=obs[i].Mp[1];
                ssat[obs[i].sat-1].mp[2]=obs[i].Mp[2];
               // obs[i].el=azel_[1+i*2];

            ssat[obs[i].sat-1].snr[0]=obs[i].SNR[0];
            if (!vsat[i]) continue;
            ssat[obs[i].sat-1].vs=1;
            //if(sol->SPPlabel&SPP_Lack){
            //    ssat[obs[i].sat-1].resp[0]=0;
            //    ssat[obs[i].sat-1].dion=0;
            //    ssat[obs[i].sat-1].dtrop=0;
            //}

            //else{
                ssat[obs[i].sat-1].resp[0]=resp[i];
                ssat[obs[i].sat-1].dion=dion[i];
                ssat[obs[i].sat-1].dtrop=dtrop[i];
