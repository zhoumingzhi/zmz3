...
static double q_gamma(double a, double x, double logga);
static double p_gamma(double a, double x, double logga)
{
    double y,w;
    int n;
    
    if (x==0.0) return 0.0;
    if (x>=a+1.0) return 1.0-q_gamma(a,x,logga);
    w=exp(-x+a*log(x)-logga)/a; y=w;
    for (n=1;n<100;n++) {
        y+=(w*=x/(a+n));
...
static double q_gamma(double a, double x, double logga)
{
    ...
    if (x<a+1.0) return 1.0-p_gamma(a,x,logga);
    w=exp(-x+a*log(x)-logga); y=w/lb;
    for (n=2;n<100;n++) {
        lc=((n-1-a)*(lb-la)+(n+x)*lb)/n; la=lb; lb=lc;
        y+=(w*=(n-1-a)/n)/(la*lb);
                   ...
                   const double *NC, int n)
{
    ...
    if ((info=filter(rtk->x,rtk->P,H,v,R,rtk->nx,n,NULL,NULL))) {
...
{
    double C1,C2,B1,v1,BC,v,vc,*NC,*var,lam_NL=lam_LC(1,1,0);
    ...
    for (i=0;i<n;i++) {
        ...
        B1=(rtk->x[j]-rtk->x[k]+C2*lam[1]*NW[i])/lam_NL; 
        N1=ROUND(B1);
        ...
        BC=C1*lam[0]*N1+C2*lam[1]*(N1-NW[i]);
...
{
    double C1,C2,*B1,*N1,*NC,*D,*E,*Q,s[2],lam_NL=lam_LC(1,1,0);
    ...
    for (i=0;i<n;i++) {
        ...
        B1[m]=(rtk->x[j]-rtk->x[k]+C2*lam[1]*NW[i])/lam_NL;
        N1[m]=ROUND(B1[m]);
    ...
    for (i=0;i<m;i++) {
        NC[i]=C1*lam[0]*N1[i]+C2*lam[1]*(N1[i]-NW[i]);
    }
    /* fixed solution */
    stat=fix_sol(rtk,sat1,sat2,NC,m);
